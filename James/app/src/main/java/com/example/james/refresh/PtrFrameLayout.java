...
//            Log.d(LOG_TAG, "onMeasure frame: width: %s, height: %s, padding: %s %s %s %s",
...
/*                Log.d(LOG_TAG, "onMeasure content, width: %s, height: %s, margin: %s %s %s %s",
                        getMeasuredWidth(), getMeasuredHeight(),
                        lp.leftMargin, lp.topMargin, lp.rightMargin, lp.bottomMargin);
                Log.d(LOG_TAG, "onMeasure, currentPos: %s, lastPos: %s, top: %s",
    ...
    private void layoutChildren() {
        ...
        if (mHeaderView != null) {
            ...
            if (DEBUG && DEBUG_LAYOUT) {
//                Log.d(LOG_TAG, "onLayout header: %s %s %s %s", left, top, right, bottom);
            }
        }
        if (mContent != null) {
            if (isPinContent()) {
            ...
            if (DEBUG && DEBUG_LAYOUT) {
//                Log.d(LOG_TAG, "onLayout content: %s %s %s %s", left, top, right, bottom);
            }
...
//                    Log.v(LOG_TAG, "ACTION_MOVE: offsetY:%s, currentPos: %s, moveUp: %s, canMoveUp: %s, moveDown: %s: canMoveDown: %s", offsetY, mPtrIndicator.getCurrentPosY(), moveUp, canMoveUp, moveDown, canMoveDown);
                }
    ...
    private void updatePos(int change) {
        ...
        if ((mPtrIndicator.hasJustLeftStartPosition() && mStatus == PTR_STATUS_INIT) ||
            ...
            if (DEBUG) {
//                Log.i(LOG_TAG, "PtrUIHandler: onUIRefreshPrepare, mFlag %s", mFlag);
            }
        }
        ...
        if (DEBUG) {
//            Log.v(LOG_TAG, "updatePos: change: %s, current: %s last: %s, top: %s, headerHeight: %s",
//                    change, mPtrIndicator.getCurrentPosY(), mPtrIndicator.getLastPosY(), mContent.getTop(), mHeaderHeight);
    ...
    final public void refreshComplete() {
        ...
        if (delay <= 0) {
            ...
            if (DEBUG) {
//                Log.d(LOG_TAG, "performRefreshComplete after delay: %s", delay);
            }
        }
    }
    ...
    private void performRefreshComplete() {
        ...
        if (mScrollChecker.mIsRunning && isAutoRefresh()) {
            // do nothing
            if (DEBUG) {
//                Log.d(LOG_TAG, "performRefreshComplete do nothing, scrolling: %s, auto refresh: %s",
...
//                            finish, mStart, mTo, mPtrIndicator.getCurrentPosY(), curY, mLastFlingY, deltaY);
        ...
        private void finish() {
            if (DEBUG) {
//                Log.v(LOG_TAG, "finish, currentPos:%s", mPtrIndicator.getCurrentPosY());
            }
            reset();
            onPtrScrollFinish();
        }
        ...
        public void tryToScrollTo(int to, int duration) {
            ...
            if (DEBUG) {
//                Log.d(LOG_TAG, "tryToScrollTo: start: %s, distance:%s, to:%s", mStart, distance, to);
            }
            removeCallbacks(this);
